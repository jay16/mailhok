  ...
  window.ShopCart = {
    chkTotal: function() {
      ...
      $(".package").each(function() {
        var id, json, name, num, price, quantity, unit;
        id = parseInt($(this).find(".id:first").val());
        name = $(this).find(".name:first").html();
        num = parseInt($(this).find(".num:first").val());
        unit = $(this).find(".unit:first").val();
    ...
    plus: function(klass, id, price) {
      var $quantity, count, input_id;
      input_id = klass + "_" + id;
    ...
    initShopCart: function() {
      ...
      if (obj["package"]) {
        return $("#package_" + obj["package"]).find(".plus").trigger("click");
      }
    }
  };
